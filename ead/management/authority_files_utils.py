...
def load_authority_file(cursor, path_to_authority_files, filename, auth_file_to_entity_concept_mapping):
    ...
    try:
        with open(filepath, 'rU') as f:
            ## add newly expected ARLABEL column that should hold Arabic prefLabels
            rows = unicodecsv.DictReader(f, fieldnames=['CONCEPTID','PREFLABEL','ARLABEL','ALTLABELS','PARENTCONCEPTID','CONCEPTTYPE','PROVIDER'], 
            ...
            for row in rows:
                try:
                    if 'MISSING' in row:
                        ...
                        ## if present, add the text in ARLABEL as prefLabel with language = 'ar'. Hard-coding solution for EAD
                        if row[u'ARLABEL'] != '':
                            concept.addvalue({'value':row[u'ARLABEL'], 'language': 'ar', 'type': 'prefLabel', 'category': 'label'}) 
